...
public class KakaoLogin {
	...
	// redirect_uri: 카카오 로그인 인증의 결과를 전달받을 콜백 URL(URL 인코딩) 애플리케이션을 등록할 때 Callback
	// URL에 설정한 정보
	...
	public OAuth2AccessToken getAccessToken(HttpSession session, String code, String state) throws IOException {
		...
		if (StringUtils.pathEquals(sessionState, state)) {
			OAuth20Service oauthService2 = new ServiceBuilder().apiKey(CLIENT_ID).apiSecret(CLIENT_SECRET)
					.callback(REDIRECT_URI).state(state).build(KakaoOAuthApi.instance());
	...
	public String getUserProfile(OAuth2AccessToken oauthToken) throws IOException {
		OAuth20Service oauthService = new ServiceBuilder().apiKey(CLIENT_ID).callback(REDIRECT_URI)
				.build(KakaoOAuthApi.instance());
